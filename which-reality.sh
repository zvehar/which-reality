...
if [[ $1 == "deploy" ]]; then
  read -rp "Enter your User ID: " YOUR_USER_ID
  read -rsp "Enter a deployment password: " DEPLOY_PASS
  echo -e "\n\033[32mCreating the ${YOUR_USER_ID} deployment user...\033[0m"
  if az webapp deployment user set --user-name "${YOUR_USER_ID}" --password "${DEPLOY_PASS}" 1>/dev/null; then exit 1; fi
  ...
  if [[ $(git remote | grep -q azure) ]]; then git remote remove azure; fi
